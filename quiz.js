...
function getQuestion() {
  ...
  selectedOption.forEach(function (input, i) {
    ...
    // Hou comment: consider using a more descriptive variable name like "answerContainer". Abbreviated variable names are hard to read and understand.
...
function handleNextQuestion() {
  ...
  if (ans == questions[currentQuestionIndex].option[0]) {
    score += 5;
    // Hou comment: alert() is a design anti-pattern, so I'd try to avoid using it in general. Instead, you could display the message on the page via DOM manipulation.
    alert('correct answer');
    // Hou comment: Remove all instances of console.log from your scripts!
...
// Hou comment: since seconds variable is only needed inside of the timeLapse function you could move it inside of timeLapse function
let seconds = 0;
function timeLapse() {
  seconds += 60;
